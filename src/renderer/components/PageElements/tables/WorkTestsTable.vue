      ...
      <tr v-for="test in data" v-bind:key="test.IdPrueba">
        <td class="pt-3-half" @focus="hidePanel">
          <i
            ...
            v-on:click="deleteRow(test.IdPrueba)"
        ...
        <td class="noMargins">
          <input
            type="text"
            v-model="test.Descripcion"
            class="inputInTd"
            @change="trackChanges($event, test.IdPrueba, 'Descripcion')"
          ...
          >
          <div
            v-if="canShow(test.IdPrueba)"
            class="list-group myTypeahead"
            style="position:absolute; left:0px; top: 48px; width: 200px; z-index=1;"
          >
            ...
            <span
              ...
              @click="selectTestDescription"
            >Prueba de bizcocho</span>
            <span class="list-group-item clickable" @click="selectTestDescription">Prueba de diente</span>
            <span
              class="list-group-item clickable"
              @click="selectTestDescription"
            >Prueba de estructura</span>
            <span
              class="list-group-item clickable"
              @click="selectTestDescription"
            >Prueba de plástico</span>
        ...
        <td class="noMargins">
          <input
            ...
            class="inputInTd"
            id="fSalida"
            v-model="test.FechaSalida"
            @blur="changedDate($event)"
            @change="trackChanges($event, test.IdPrueba, 'FechaSalida')"
            @focus="hidePanel"
            :disabled="$attrs.disabled === true"
          >
        </td>
        <td class="noMargins">
          <select
            class="inputInTd"
            v-model="test.IdTurnoFechaSalida"
            @change="trackChanges($event, test.IdPrueba, 'IdTurnoFechaSalida')"
            @focus="hidePanel"
            :disabled="$attrs.disabled === true"
          >
            <option value></option>
            <option
              v-for="shift in deliveryShifts"
              v-bind:key="shift.IdTurno"
              v-bind:value="shift.IdTurno"
            >{{shift.Descripcion}}</option>
          </select>
        </td>
        <td class="noMargins">
          <input
            type="date"
            class="inputInTd"
            v-model="test.FechaEntrada"
            @change="trackChanges($event, test.IdPrueba, 'FechaEntrada')"
            @focus="hidePanel"
            :disabled="$attrs.disabled === true"
          >
        </td>
        <td class="noMargins">
          <select
            class="inputInTd"
            v-model="test.IdTurnoFechaEntrada"
            @change="trackChanges($event, test.IdPrueba, 'IdTurnoFechaEntrada')"
            @focus="hidePanel"
            :disabled="$attrs.disabled === true"
          >
            <option value></option>
        ...
        <td class="noMargins">
          <input
            type="text"
            v-model="test.Comentario"
            class="inputInTd"
            @change="trackChanges($event, test.IdPrueba, 'Comentario')"
            @focus="hidePanel"
            :disabled="$attrs.disabled === true"
          >
        </td>
      ...
      <tr v-if="$attrs.disabled !== true">
        ...
        <td class="noMargins">
          <input
            ...
            :class="{'bg-danger text-white animated flash': $v.newRow.descripcion.$anyError}"
          >
          <div
            v-if="canShow('newDescripcion')"
            class="list-group myTypeahead"
            style="position:absolute; left:0px; top: 48px; width: 200px; z-index=1;"
          >
            <span class="list-group-item clickable" @click="selectTestDescription">Cubeta</span>
        ...
        <td class="noMargins">
          <input
            ...
            id="newFechaSalida"
            @focus="hidePanel"
            @blur="updateIfDateIsValid($event)"
            @input="updateIfDateIsValid($event)"
            v-model="$v.newRow.fechaSalida.$model"
            :class="{'bg-danger text-white animated flash': isInvalidNewOutDate}"
          >
        </td>
        <td class="noMargins">
          <select
            class="inputInTd"
            ref="newTurnoSalida"
            v-model="$v.newRow.idTurnoFechaSalida.$model"
          >
        ...
        <td class="noMargins">
          <input
            ...
            class="inputInTd"
            id="newFechaEntrada"
            v-model="$v.newRow.fechaEntrada.$model"
            @blur="updateIfDateIsValid($event)"
            @input="updateIfDateIsValid($event)"
            :class="{'bg-danger text-white animated flash': isInvalidNewInDate}"
          >
        </td>
        <td class="noMargins">
          <select
            class="inputInTd"
            ref="newTurnoEntrada"
            v-model="$v.newRow.idTurnoFechaEntrada.$model"
          >
            <option selected="selected"></option>
            <option
              v-for="shift in deliveryShifts"
        ...
        <td class="noMargins">
          <input
            type="text"
            class="inputInTd"
            v-model="$v.newRow.comentario.$model"
            :class="{'bg-danger text-white animated flash': $v.newRow.comentario.$error && !allRowEmpty}"
            @blur="addLastRow()"
          >
        </td>
      </tr>
	...
	computed: {
		allRowEmpty: function() {
			return (
				...
				this.$v.newRow.fechaSalida.$model.length === 0 &&
				this.$v.newRow.idTurnoFechaSalida.$model === '' &&
				this.$v.newRow.fechaEntrada.$model.length === 0 &&
				this.$v.newRow.idTurnoFechaEntrada.$model === '' &&
				this.$v.newRow.comentario.$model.length === 0
			)
		}
	},
	created() {
