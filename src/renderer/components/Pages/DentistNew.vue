...
export default {
	...
	methods: {
		save: async function(url) {
			log.info('Saving the new dentist on progress')
			this.$v.data.$touch()
			if (this.$v.data.$anyError) {
				...
				log.debug('A validator is preventing to save')
				return false
			}
			log.debug('No validator is preventing to save')
			await this.dentistService.insertDentist(this.data)
			this.$v.$reset()
			if (!url) {
				log.debug('Return to the previous page from saving the dentist')
				this.$router.go(-1)
			} else {
				log.debug(`Return to ${url} from saving the dentist`)
				this.$router.push({
					path: url
				})
			}
