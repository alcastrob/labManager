<template>
  <div class="box invisible" id="labelMetalCeramica">
    <div style="position: absolute; top: +60px; left: 0px; z-index: 10;" class="labelTitle">
      {{labelName}}
    </div>
    <div style="position: absolute;" class="labelTitle">
      <img src="~@/assets/metal-ceramica.jpg" style="width: 250px; height: 70px; z-index: 0; position: absolute; left: +50px;">
    </div>
...
export default {
  ...
  methods: {
    print (label) {
      ...
      this.$forceUpdate()
      const d = new Printd()
      d.print( document.getElementById('labelMetalCeramica'), this.cssText)
    }
  },
  computed: {
    labelName: function() {
